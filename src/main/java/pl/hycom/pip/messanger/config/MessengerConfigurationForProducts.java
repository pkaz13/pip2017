...
import pl.hycom.pip.messanger.handler.ReturnThreeProductsHandler;
...
@Configuration
public class MessengerConfigurationForProducts {
	...
	@Bean
	public ReturnThreeProductsHandler returnThreeProductsHandler() {
		return new ReturnThreeProductsHandler(sendClient());
	}

	@Bean
	public MessengerReceiveClient receiveClient() {
		return MessengerPlatform.newReceiveClientBuilder(appSecret, verifyToken)
				.onTextMessageEvent(returnThreeProductsHandler())
				.build();
	}
